...
// Check ticket reservations
                ...
                if ($orderTotalAmount == 0) {
                    ...
                    $paymentGateway = $services->getPaymentGateways(array('gatewayFactoryName' => $gatewayFactoryName), true)->getQuery()->getOneOrNullResult();
...
// Sets the amount to be paid
                $orderamount = intval($orderTotalAmount); // intval(bcmul($orderTotalAmount, 100));

                //check if there is a payment with the same order
                $payments = $em->getRepository(Payment::class)->findBy(array('order'=>$order->getId()));
                if(empty($payments)){
                ...
                if ($this->isGranted("ROLE_ATTENDEE")) {
                    ...
                    $payment->setPhonenumber($form->getData()['config']['phonenumber']);
                }
                ...
                if ($this->isGranted("ROLE_ATTENDEE")) {
                    if ($request->request->get('payment_gateway') == "offline") {
                        ...
                        //$gatewayFactoryName == 'mpesa' ? 'mpesa_pay' :
                    }
                } else {
                    $captureToken = $this->get('payum')->getTokenFactory()->createCaptureToken(
               ...
               /*  if($gatewayFactoryName == 'mpesa'){
                    $this->addFlash("error", "No way hosse");
                    // $this->get('payum')->get
                    return $this->redirectToRoute("dashboard_attendee_checkout", ['orderReference' => $payment->getOrder()->getReference()]);
                } */
